...
import os
os.environ["NSYS_NVTX_PROFILER_REGISTER_ONLY"] = "0"  # be sure to profile C++
for paramcount in ['130m','370m']:
   for ctx_len in [1000,2000,4000]:
        cmd=f"""sudo /usr/local/cuda/bin/nsys profile --trace=cuda --force-overwrite=true --output={paramcount}_{ctx_len} \
   $(which python3) benchmarks/benchmark_generation_mamba_simple.py \
